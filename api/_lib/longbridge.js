...
function normalizeSdkError(error, scope = '长桥数据') {
  ...
  if (/OPRA|option|us options/i.test(message) && /quote|permission|access|auth|not.?authorized|forbidden/i.test(message)) {
    return `${scope}需要开通 OPRA US Options Quotes（OpenAPI）权限；长桥 App/Web 行情权限和 OpenAPI 权限是分开的。`;
  }
...
export async function fetchLongbridgeOptionQuote(contractSymbol, credentials) {
  ...
  try {
    const cliQuote = await requestLongbridgeCliOptionPrice(contractSymbol);
    if (cliQuote) return cliQuote;
  } catch (error) {
    errors.push(normalizeSdkError(error, '长桥 CLI 期权报价'));
  }
